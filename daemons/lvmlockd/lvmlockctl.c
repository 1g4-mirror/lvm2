...
static void print_usage(void)
{
	...
	printf("--set-lock <vgname> --lv-uuid <uuid> --lock-mode <mode> [--foreign]\n");
	printf("      Test only (daemon_test mode): force a lock mode for an LV resource.\n");
	printf("      --foreign: also mark the resource as held by a remote node so that\n");
	printf("      subsequent lock attempts on it return EAGAIN (as if another node holds it).\n");
}
...
static int read_options(int argc, char *argv[])
{
	...
	static const struct option _long_options[] = {
		...
		{"foreign",         no_argument,       0,  OPT_FOREIGN   },
		{0, 0, 0, 0 }
	};
