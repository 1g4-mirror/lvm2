...
#define MSDOS_PART_MAGIC 0xAA55
#define MSDOS_PART_MAGIC_OFFSET UINT64_C(0x1FE)
#define MSDOS_PART_OFFSET UINT64_C(0x1BE)
#define MSDOS_PART_TYPE_GPT_PMBR UINT8_C(0xEE)

#define GPT_HEADER_OFFSET_LBA 0x01
#define GPT_PART_MAGIC 0x5452415020494645UL /* "EFI PART" string */
#define GPT_PART_ENTRIES_FIELDS_OFFSET UINT64_C(0x48)
...
static int _has_gpt_partition_table(struct device *dev)
{
	...
	struct {
		...
		uint8_t skip[GPT_PART_ENTRIES_FIELDS_OFFSET - sizeof(uint64_t)];
	...
	if (!dev_read_bytes(dev, GPT_HEADER_OFFSET_LBA * lbs, sizeof(gpt_header), &gpt_header))
		return_0;

	/* the gpt table is always written using LE on disk */

	if (le64_to_cpu(gpt_header.magic) != GPT_PART_MAGIC)
		return_0;
...
static int _has_partition_table(struct device *dev)
{
	...
	unsigned p;
	struct {
		uint8_t skip[MSDOS_PART_OFFSET];
	...
	if (buf.magic == xlate16(MSDOS_PART_MAGIC)) {
		for (p = 0; p < 4; ++p) {
			...
			if (buf.part[p].nr_sects) {
				/* If this is GPT's PMBR, then also check for GPT partitions. */
				if (buf.part[p].sys_ind == MSDOS_PART_TYPE_GPT_PMBR)
