 ...
 *   - io_uring   (when IORING_OP_IOCTL is defined, requires Linux 6.5+)
 */
...
static void *_worker_fn(void *arg)
{
	...
	struct _dm_work_item *item;
	unsigned cmd;

	pthread_mutex_lock(&ctx->lock);
	while (1) {
		while (dm_list_empty(&ctx->pending) && !ctx->shutdown)
		...
		item = dm_list_item(dm_list_first(&ctx->pending),
				    struct _dm_work_item);
		dm_list_del(&item->list);
		pthread_mutex_unlock(&ctx->lock);

		/* Execute the ioctl outside the lock. */
		cmd = dm_task_get_ioctl_cmd(item->dmt);
		item->dmt->ioctl_errno = 0;
		item->result = ioctl(ctx->base.fd, cmd, item->dmt->dmi.v4);
		if (item->result < 0)
			item->dmt->ioctl_errno = errno;
...
struct dm_async_ctx *_dm_async_ctx_alloc_threads(int fd, unsigned max_parallel)
{
		       ...
		       void **userdata_out, int *result_out)
{
	...
	item = (struct _dm_work_item *)(uintptr_t)cqe->user_data;
...
struct dm_async_ctx *_dm_async_ctx_alloc_uring(int fd, unsigned max_parallel)
{
