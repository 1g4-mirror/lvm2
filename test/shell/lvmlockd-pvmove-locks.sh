...
# Inject foreign-EX.  Because no real lk entry exists after deactivation,
# set_lock creates a synthetic one (LD_LF_TEST_INJECTED) so that --info can
# display the injected mode via format_info_lk.
lvmlockctl --set-lock $vg --lv-uuid "$lv1_uuid" --lock-mode ex --foreign
lvmlockctl --info | tee out
grep "LK LV ex $lv1_uuid" out

# Activation must fail: lm_lock() returns EAGAIN because test_foreign is set,
# simulating another cluster node holding the EX lock.
not lvchange -ay $vg/$lv1
check inactive $vg $lv1

# Clear the foreign state: synthetic lk is removed, resource freed on next
# lockspace loop pass.
lvmlockctl --set-lock $vg --lv-uuid "$lv1_uuid" --lock-mode un
...
# segments.  If another cluster node already holds EX (--foreign
...
# Inject foreign EX: simulates another cluster node holding the lock
# while lv1 is not active locally.  Our lockd_lv("ex") call in pvmove
# setup gets EAGAIN (test_foreign is set on the resource).
lvmlockctl --set-lock $vg --lv-uuid "$lv1_uuid" --lock-mode ex --foreign
lvmlockctl --info | tee out
grep "LK LV ex $lv1_uuid" out
...
# Clear the foreign lock: synthetic entry removed, lv1 accessible again.
lvmlockctl --set-lock $vg --lv-uuid "$lv1_uuid" --lock-mode un
