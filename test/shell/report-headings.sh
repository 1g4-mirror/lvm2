...
aux lvmconf 'report/ids_in_headings=0'

pvs -o $OPTS > out
grep "BA Start Start Start" out

pvs --idsinheadings -o $OPTS > out
...
pvs --rows --idsinheadings -o $OPTS > out
grep "pv_ba_start 0" out
grep "seg_start 0" out
grep "seg_start_pe 0" out


aux lvmconf 'report/ids_in_headings=1'
...
# if using --nameprefixes, the ids_in_headings=1 is ignored
pvs --nameprefixes -o $OPTS > out
